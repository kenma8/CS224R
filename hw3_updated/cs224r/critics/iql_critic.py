...
class IQLCritic(BaseCritic):
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE START HERE ###
        q_est = torch.gather(self.q_net_target(ob_no), 1, ac_na.type(torch.int64).unsqueeze(1)).squeeze(1)
        v_est = self.v_net(ob_no).squeeze(1)
        diff = q_est - v_est
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ### YOUR CODE START HERE ###
        q_est = torch.gather(self.q_net(ob_no), 1, ac_na.type(torch.int64).unsqueeze(1)).squeeze(1)
        v_est = self.v_net(next_ob_no).squeeze(1).detach() * (1 - terminal_n)
        loss = self.mse_loss(q_est, reward_n + self.gamma * v_est)
        ### YOUR CODE END HERE ###
